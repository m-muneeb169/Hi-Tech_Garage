...
//               </button>
//             </div> */}
//             {/* Settings Button Adjusted to Align with Chatbot */}
// <div className="fixed bottom-[26px] right-28 z-50">
//   <button
//     onClick={handleSettings}
//     className="bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105"
//     title="Settings"
//   >
//     <FiSettings size={24} />
//   </button>
...
import { db } from '../firebase';
import { doc, updateDoc, arrayUnion } from 'firebase/firestore';
import { FiSettings, FiTool, FiHome, FiAlertTriangle } from 'react-icons/fi';
  ...
  return (
    <div className={`relative ${showModal ? 'overflow-hidden' : ''}`}>
      ...
      <div className={`page-header relative flex items-center justify-center overflow-hidden ${showModal ? 'filter blur-sm pointer-events-none' : ''}`}>
        ...
        <div className="absolute inset-0 bg-black/30"></div>
        <div className="container mx-auto px-4 relative z-10">
          <p className="text-white text-4xl font-bold tracking-wider">Bookings</p>
        </div>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="bg-white rounded-3xl shadow-2xl border-[3px] border-blue-100 max-w-3xl w-full p-10 space-y-10 relative">

            {/* Heading */}
            <h2 className="text-4xl font-bold text-center text-blue-900 tracking-wide">
              Select a Service
            </h2>
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-6">
              <button
                onClick={() => handleButtonClick('Maintenance At Home')}
                className="flex-1 py-6 px-4 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-2xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 flex flex-col items-center"
              >
                <FiHome className="text-3xl mb-1" />
                <span className="text-lg font-semibold">On-Site Maintenance</span>
                <span className="text-sm text-blue-100">At Your Location</span>
              </button>

              <button
                onClick={() => handleButtonClick('Repair In Workshop')}
                className="flex-1 py-6 px-4 bg-gradient-to-r from-indigo-600 to-blue-900 text-white rounded-2xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 flex flex-col items-center"
              >
                <FiTool className="text-3xl mb-1" />
                <span className="text-lg font-semibold">Workshop Repair</span>
                <span className="text-sm text-blue-100">Visit Workshop</span>
              </button>
            </div>
            ...
            <div className="flex justify-center">
              <button
                ...
                className="w-full sm:w-1/2 py-6 px-4 bg-gradient-to-r from-red-600 to-red-800 text-white rounded-2xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 flex flex-col items-center"
              >
                <FiAlertTriangle className="text-3xl mb-1" />
                <span className="text-lg font-semibold">Emergency</span>
                <span className="text-sm text-red-100">Get Immediate Help</span>
              </button>
            </div>

            {/* Settings Button */}
            <div className="absolute bottom-4 right-24 z-50">
              <button
                onClick={handleSettings}
                className="bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105"
                title="Settings"
