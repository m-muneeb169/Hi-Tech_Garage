...
//         // **NEW CODE: Add the selected time slot to bookedSlots**
//         const selectedTimeSlot = latestOrder?.usertimeselected;
//         const currentBookedSlots = workshopData.bookedSlots || [];
        
//         let updatedBookedSlots = [...currentBookedSlots];
        
//         // Add the time slot ID to booked slots if it exists and isn't already booked
//         if (selectedTimeSlot && selectedTimeSlot.id && !currentBookedSlots.includes(selectedTimeSlot.id)) {
//           updatedBookedSlots.push(selectedTimeSlot.id);
//         }

//         await updateDoc(workshopRef, {
//           orders: [...workshopOrders, confirmedOrder],
//           bookedSlots: updatedBookedSlots // Update the booked slots
//         });
//       }
...
const Address = () => {
  ...
  //       await updateSlotAvailability(workshopId, selectedTimeSlot?.id);
  //     }
  ...
  try {
    ...
    const confirmedOrder = {
      ...
      userId: user.uid,
      orderId: orderId,
      createdAt: new Date().toISOString(), // <-- this line adds the current timestamp
    };
    ...
    if (workshopId) {
      ...
      await updateDoc(workshopRef, {
        orders: [...workshopOrders, confirmedOrder], // ⬅️ this now includes createdAt
        bookedSlots: updatedBookedSlots,
      });
