        ...
        try {
          ...
          if (workshopDoc.exists()) {
            ...
            const availableSlots = (data.timeSlots || [])
              .filter(slot => slot.available !== false)  // Only include slots with available: true or undefined
              .map((slot, index) => ({
                ...slot,
  ...
  const updateSlotAvailability = async (workshopId, slotId) => {
    try {
      ...
      if (workshopSnap.exists()) {
        ...
        console.log("Time slot marked unavailable.");
      }
    } catch (error) {
            ...
            <div className="flex items-center justify-center space-x-8 mb-8">
              <div className={`flex items-center space-x-2 ${selectedWorkshop ? 'text-blue-600' : 'text-gray-400'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedWorkshop ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}>
                  {selectedWorkshop ? <CheckCicle2Icon className="w-5 h-5" /> : <MapPinIcon className="w-5 h-5" />}
                </div>
